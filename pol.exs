...
defmodule Pol do
  ...
  defp _div_pol(a,b,q) when length(a) < length(b) do
    {Enum.reverse(q) |> remove_zeros, a}  ##added check
  end
  ...
  defp _add(a,b) do
    ...
    ret = Enum.zip(a,b) |> Enum.map(fn {a,b} -> a + b end)
  ...
  def remove_zeros([h | t]) when h < 0.0000001 and h > -0.0000001 do
    remove_zeros(t)
  end

  def remove_zeros([0 | t]) do
    remove_zeros(t)
  end
